...
# Sylvain Glemin
# sylvain.glemin@univ-rennes1.fr
...
#' where Tws and Tsw are the "True" SFSs, B = 4*Ne*b is the population-scaled gBGC, mut_bias the mutation bias towards AT, and j/n the frequency class j.
...
sum_of_squares <- function(par,WS,SW) {
  if(length(par)!=4) {
...
#' LS <- least_square(sfsWS,sfsSW)
#' LS$B # population-scaled gBGC
#' LS$mutbias # mutation bias


least_square <- function(WS,SW,Bmin=-100,Bmax=100,Mmin=-5,Mmax=5,MAXIT=100,FACTR=10^7,LMM=20,VERBOSE=0) {
  ...
  minSSE <- optim(init,sum_of_squares,WS=WS,SW=SW,lower = inf,upper = sup,method="L-BFGS-B",control=list(parscale=abs(init),maxit=MAXIT,factr=FACTR,lmm=LMM,trace=VERBOSE))
  return( list("B"=minSSE$par[1],"mutbias"=exp(minSSE$par[2]),"e1"=minSSE$par[3],"e2"=minSSE$par[4]) )
}