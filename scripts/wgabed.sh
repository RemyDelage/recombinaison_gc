...
#SBATCH --mem=150G
#SBATCH --output=wgabed_20240422.log
...
input_file=$data_dir"arabidopsis_genomes_alignment.maf.gz"
...
for i in "${chromosomes[@]}"; do
	python "$scripts_dir"maf_to_bed_v2.py -i "$input_file" -r GCF_000001735_Arabidopsis_thaliana -c "$i" -s "$species_names_file" | sort -k1,1 -k2,2n | bgzip -c > "$results_dir"arabidopsis_"$i".wga.bed.gz
done
